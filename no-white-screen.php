...
class No_White_Screen_Of_Death
{
    ...
    private function __construct()
    {
        ...
        add_action('all', [$this, 'init'], 1);
        add_action('all', [$this, 'init'], 9999);
    }
    ...
    private function dump($message, $type, $trace, $err_file = false, $err_line = false, $color = '#AAA')
    {
        ...
        if (ini_get('log_errors')) {
            ...
            $message = 'Backtrace from '.$type.' "'.$message.'"'.$file_pos.''.implode(' | ', $items);
            error_log($message);
        }
    ...
    {
        if (defined('WP_DEBUG_CORE') && !WP_DEBUG_CORE) {
            return;
        }

        error_reporting(E_ALL); // Not sure if this is needed, but we'll add it!

        set_error_handler([$this, 'process_error']);
        set_exception_handler([$this, 'process_exception']);
    }
}
